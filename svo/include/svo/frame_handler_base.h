...
enum class Stage {
  ...
  kRelocalization,    ///< Stage when SVO loses tracking and it tries to relocalize
  kReinitialize      ///< Stage when SVO failed to relocalize and needs to initialize new keypoint
};
...
protected:
  ...
  bool set_reset_;              //!< (Not used in this version of the code) Flag that the user can set. Will reset the system before the next iteration.
