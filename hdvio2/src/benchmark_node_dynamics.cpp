...
public:
  ...
  void traceExternalForce(const Eigen::Matrix<double, 3, 1>& F, const double ts);
...
private:
  ...
  std::ofstream trace_external_force_;
  ...
  const std::string kStampedForceEst = "stamped_external_force_estimate";
  ...
  : SvoInterface(type, nh, pnh)
{
  ...
  safeCreateTrace(kStampedForceEst + save_suf + "." + kExt, &trace_external_force_);
  trace_external_force_.precision(20);
...
void BenchmarkNode::traceExternalForce(const Eigen::Matrix<double, 3, 1>& F, const double ts)
{

  trace_external_force_ << ts << " "; 
  trace_external_force_ << F.x() << " " << F.y() << " "  << F.z() << std::endl;
}
...
void BenchmarkNode::runBenchmark(const std::string& dataset_dir)
{
  ...
  // Load dynamics measurements messages.
  if (dynamics_handler_)
  {
    std::string dynamics_filename(dataset_dir + "/data/dynamics.txt");
    Eigen::Matrix4d T_B_I(ncam_->get_T_B_I().getTransformationMatrix());
    Eigen::Matrix3d R_B_I(T_B_I.block(0,0,3,3));

    if (!dynamics_handler_->loadDynamicsMeasurementsFromFile(
      dynamics_filename, dynamics_dataset_, R_B_I))
    {
      return;
    }
  }
    ...
    if (svo_->stage() == Stage::kTracking || svo_->getBundleAdjuster())
    {
      ...
      if (!trace_only_kf_ || nframe.isKeyframe())
      {
        ...
        if (svo_->getBundleAdjuster())
        {
          ...
          int64_t vi_state_ts_ns = ba_ptr->getLastStateTimestamp();
          double vi_state_ts_s = vi_state_ts_ns * 1e-9;
          Twb = vi_state.get_T_W_B();

          if (ba_ptr->lastOptimizedBundleId() > 0)
          {
            traceSpeedBias(vi_state, img_id, vi_state_ts_s);
            traceIdTimePose(Twb, img_id, vi_state_ts_s);
            tracePose(Twb, vi_state_ts_s);

            bool has_external_force = false;
            Eigen::Matrix<double, 3, 1> external_force;
            if (dynamics_handler_->options_.use_dynamics)
            {
              BundleId last_optimized_nframe_min_1 = ba_ptr->lastOptimizedBundleId() - 1;
              if (last_optimized_nframe_min_1 >= 0)
              {
                has_external_force = ba_ptr->logExternalForce(last_optimized_nframe_min_1, external_force);
              }

              if (has_external_force)
              {
                traceExternalForce(external_force, vi_state_ts_s);
              }
            }
          }
        }
...
void BenchmarkNode::runKittiBenchmark(const std::string& dataset_dir)
{
  ...
  while (dataset_fs.good() && !dataset_fs.eof() && rclcpp::ok())
  {
    ...
    publishResults({ img_l_8uC1, img_r_8uC1 }, rclcpp::Clock().now().nanoseconds());
