...
bool FrameHandlerBase::addImageBundle(const std::vector<cv::Mat>& imgs, const uint64_t timestamp)
{
  ...
  if (!last_frames_)
  {
    // at first iteration add IMU measurements to initialize orientation
    ImuMeasurements imu_measurements = imu_handler_->getMeasurementsCopy();
    const size_t n_imu = imu_measurements.size();
    frame_bundle->imu_measurements_.resize(6, n_imu);
    frame_bundle->imu_timestamps_ns_.resize(1, n_imu);

    size_t idx = n_imu;
    ImuMeasurements::iterator it=imu_measurements.begin();
    for(; it!=imu_measurements.end(); ++it)
    {
      Eigen::Matrix<double, 6, 1> meas; // [acc, gyr]
      meas.head<3>() = it->linear_acceleration_;
      meas.tail<3>() = it->angular_velocity_;

      idx--;
      frame_bundle->imu_measurements_.block<6,1>(0, idx) = meas;
      frame_bundle->imu_timestamps_ns_(0, idx) = static_cast<int64_t>(it->timestamp_ * 1e9);
    }
  }
...
bool FrameHandlerBase::addFrameBundle(const FrameBundlePtr& frame_bundle)
{
  ...
  if (bundle_adjustment_)
  {
    ...
    // Do not reinitialize backend in case of frontend failure.
    // This will never be called in this version of the code.
    if (backend_reinit_)
    {
    ...
    if (bundle_adjustment_->getNumFrames() > 0 && have_motion_prior_)
    {
      ...
      // Do not reinitialize backend in case of frontend failure.
      // This will always be false in this version of the code.
  ...
  {
    VLOG(1) << "Tracking failed: RELOCALIZE.";
    // SVO_INFO_STREAM("Tracking failed: RELOCALIZE.");
    ...
    {
      VLOG(1) << "Relocalization failed "
              << options_.relocalization_max_trials << " times.";
      // SVO_INFO_STREAM("Relocalization failed!");

      // Comment this to switch off reinitialization.
      // stage_ = Stage::kReinitialize;
    } 
  }
...
void FrameHandlerBase::setInitialPose(const FrameBundlePtr& frame_bundle) const
{
  ...
  {
    VLOG(2) << "Set initial pose: With rotation prior";
    for (size_t i = 0; i < frame_bundle->size(); ++i)
    {
      frame_bundle->at(i)->T_f_w_ = cams_->get_T_C_B(i) * Transformation(R_imu_world_, Vector3d::Zero());
    }
    VLOG(2) << "Initial Rotation = " << std::endl << R_imu_world_.inverse() << std::endl;
  }
  else if (frame_bundle->imu_measurements_.cols() > 0)
  {
    VLOG(2) << "Set initial pose: Use inertial measurements in frame to get gravity.";
    // acceleration vector
    Eigen::Vector3d acc_B = Eigen::Vector3d::Zero();
    const int n_measurements = frame_bundle->imu_measurements_.cols();
    for (int i = 0; i < n_measurements; ++i)
    {
      acc_B += Eigen::Vector3d(
        frame_bundle->imu_measurements_(0, i), 
        frame_bundle->imu_measurements_(1, i), 
        frame_bundle->imu_measurements_(2, i) );
    }
    ...
    Transformation T_world_imu(Transformation::exp(-pose_increment));
    T_world_imu.getRotation().normalize();
    Transformation T_imu_world(T_world_imu.inverse());

    frame_bundle->set_T_W_B(T_world_imu);
    for (size_t i = 0; i < frame_bundle->size(); ++i)
    {
      frame_bundle->at(i)->T_f_w_ = cams_->get_T_C_B(i) * T_imu_world;
    }
    std::string s;
    s = "Frontend initialized with pose: p (" + 
    std::to_string(T_world_imu.getPosition().x()) + "," + std::to_string(T_world_imu.getPosition().y()) 
    + "," + std::to_string(T_world_imu.getPosition().z()) + "), q(" +
    std::to_string(T_world_imu.getEigenQuaternion().x()) + "," + std::to_string(T_world_imu.getEigenQuaternion().y()) + "," 
    + std::to_string(T_world_imu.getEigenQuaternion().z()) + "," + std::to_string(T_world_imu.getEigenQuaternion().w()) + ")";
    VLOG(2) << s;
  }
  else
  {
    VLOG(2) << "Set initial pose: set such that T_imu_world is identity.";
    for (size_t i = 0; i < frame_bundle->size(); ++i)
    {
    ...
    std::string s;
    s = "Frontend initialized with pose: p (" + 
    std::to_string(T_world_imuinit_.getPosition().x()) + "," + std::to_string(T_world_imuinit_.getPosition().y()) 
    + "," + std::to_string(T_world_imuinit_.getPosition().z()) + "), q(" +
    std::to_string(T_world_imuinit_.getEigenQuaternion().x()) + "," + std::to_string(T_world_imuinit_.getEigenQuaternion().y()) + "," 
    + std::to_string(T_world_imuinit_.getEigenQuaternion().z()) + "," + std::to_string(T_world_imuinit_.getEigenQuaternion().w()) + ")";
    VLOG(2) << s;
  }
}
