...
private:
  ...
  ros::Publisher pub_imu_vel_;
  ros::Publisher pub_imu_bias_;
  ros::Publisher pub_dynamics_meas_;
  ros::Publisher pub_dynamics_;
  ...
  void publishImuPoseVelBias(const ViNodeState& state, const int64_t timestamp, const int32_t seq);
  void publishImuPoseAndBias(const ViNodeState& state, const int64_t timestamp, const int32_t seq);
  void publishBackendLandmarks(const int64_t timestamp) const;
  void publishDynamics(
    const DynamicsMeasurements& m,
    const bool pub_body_rates,
    const Eigen::Matrix<double, 3, 1>& Fe, 
    const int64_t Fe_timestamp, const int32_t seq);

  // timestamp of the latest dynamic measurement published
  double dynamic_measurement_latest_time_ = 0.0;
};
