...
public:
  ...
  /**
   * @brief Set the dynamics handler in backend
   * @param dynamics_handler Shared DynamicsHandler with frontend
   */
  void setDynamicsHandler(const std::shared_ptr<DynamicsHandler> dynamics_handler);

  /**
   * @brief Set the thrust dynamics net in backend
   * @param dynamics_net Shared DynamicsNet with frontend
   */
  void setThrustDynamicsNet(const std::shared_ptr<DynamicsNet> dynamics_net);

  /**
   * @brief Set the torque dynamics net in backend
   * @param dynamics_net Shared DynamicsNet with frontend
   */
  void setTorqueDynamicsNet(const std::shared_ptr<DynamicsNet> dynamics_net);

  /**
   * @brief Set the dynamics net in backend
   * @param dynamics_net Shared OptimizeQuadAngVelSpline with frontend
   */
  void setOptimizeQuadAngVelSpline(const std::shared_ptr<OptimizeQuadAngVelSpline> opt_quad_angvel_spline);

  /**
   * @brief Set the groundtruth poses for initialization in backend
   * @param gt_poses Groundtruth poses
   */
  void setGroundtruthPoses(PoseMeasurements& gt_pose);
  ...
  /**
   * @brief Log external force
   * @param bundle_id
   * @param external_force
   */
  inline bool logExternalForce(int32_t bundle_id, ExternalForce& external_force) const override
  {
    return backend_.getExternalForce(bundle_id, external_force);
  }
  ...
  inline int64_t getLastStateTimestamp() const override
  {
    return last_state_timestamp_ns_;
  }
...
protected:
  ...
  int64_t before_the_last_added_frame_stamp_ns_ = 0;
  ...
  int64_t last_state_timestamp_ns_ = 0;
