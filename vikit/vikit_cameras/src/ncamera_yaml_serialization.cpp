...
bool convert<std::shared_ptr<vk::cameras::NCamera> >::decode(
  ...
  try {
    ...
    */
    ...
    vk::cameras::Transformation T_B_I;
    ...
    for (size_t camera_index = 0; camera_index < num_cameras; ++camera_index) {
      ...
      Eigen::Matrix4d T_I_C_raw;
      if (!YAML::safeGet(camera_node, "T_I_C", &T_I_C_raw)) {
        LOG(ERROR) << "Unable to get extrinsic transformation T_I_C for camera " << camera_index;
        return true;
      }

      Eigen::Matrix4d T_B_I_raw;
      if (camera_index == 0)
      {
        if (!YAML::safeGet(camera_node, "T_B_I", &T_B_I_raw)) 
        {
          LOG(ERROR) << "Unable to get extrinsic transformation T_B_I for camera " << camera_index;
          return true;
        } 
      }
      else
      {
        T_B_I_raw = T_B_I.getTransformationMatrix();
      }

      // the naming convention of svo considers that B(ody) frame = I(mu) frame 
      Eigen::Matrix4d T_B_C_raw(T_I_C_raw);
      ...
      if (camera_index == 0)
      {
        T_B_I = vk::cameras::Transformation(T_B_I_raw);
      }
    }

    // Create the ncamera and fill in all the data.
    ncamera.reset(new vk::cameras::NCamera(T_Ci_B, T_B_I, cameras, label));
