...
def run_experiments(experiment_file, num_monte_carlo_runs=None,
    ...
    base_params['cooldown_interval_sec'] = 1
    ...
    dataset_cfg = experiment_params['datasets'][0]
    ...
    # reprojector_max_n_kfs = [5, 10, 15]

    n_runs = len(detector_threshold_primary)
    # n_runs = int(len(detector_threshold_primary)) * int(len(reprojector_max_n_kfs))
    ##########################################################################

    for ni in range(n_runs):
        idx1 = ni
        # idx1 = int(ni / len(reprojector_max_n_kfs))
        # idx2 = int(ni % len(reprojector_max_n_kfs))
        ...
        #     '/' + trace_name
        dataset_trace_dir = cur_params['time'] + '_' + experiment_file + '/' + str(ni)
        ...
        # load parameters
        cur_params['detector_threshold_primary'] = detector_threshold_primary[idx1]
        # cur_params['reprojector_max_n_kfs'] = reprojector_max_n_kfs[idx2]

        # Deal with parameters in the calibration file
        calib_name = 'calib.yaml'
        if 'calib_name' in cur_params:
            calib_name = cur_params['calib_name']

        cur_params['calib_file'] = os.path.join(
            cur_params['dataset_directory'], calib_name)
        if not os.path.exists(cur_params['calib_file']):
            raise Exception('File does not exist: '+ cur_params['calib_file'])
        
        calib_params = yaml.load(open(cur_params['calib_file'], 'r'), Loader=yaml.Loader)
        # calib_params['dynamics_params']['sigma_accel_c'] = sigma_accel_c[idx2]

        with open(os.path.join(cur_params['trace_dir'], 'calib.yaml'), 'w') as f:
            yaml.dump(calib_params, f, default_flow_style=False)

        cur_params['calib_file'] = os.path.join(
            cur_params['trace_dir'], 'calib.yaml')
        ...
        #                                 'experiment.yaml'))
        with open(os.path.join(cur_params['trace_dir'], 'experiment.yaml'), 'w') as f:
            yaml.dump(cur_params, f, default_flow_style=False)
        ...
        # run evaluation
        arg1 = '--mul_trials='+str(num_monte_carlo_runs)
        subprocess.run(
            ['rosrun', 'rpg_trajectory_evaluation', 'analyze_trajectory_single.py', params_i['trace_dir'], arg1], 
            check=True)
