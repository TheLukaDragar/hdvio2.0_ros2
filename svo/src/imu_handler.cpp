...
ImuCalibration ImuHandler::loadCalibrationFromFile(const std::string& filename)
{
  ...
  if(data["imu_params"].IsDefined())
  {
    ...
    if(data["imu_params"]["correct_scale_and_axis_misalignment"].IsDefined())
    {
      calib.correct_scale_and_axis_misalignment = data["imu_params"]["correct_scale_and_axis_misalignment"].as<bool>();
      
      calib.Mg(0,0) = data["imu_params"]["Mg"][0].as<double>();
      calib.Mg(1,0) = data["imu_params"]["Mg"][1].as<double>();
      calib.Mg(1,1) = data["imu_params"]["Mg"][2].as<double>();
      calib.Mg(2,0) = data["imu_params"]["Mg"][3].as<double>();
      calib.Mg(2,1) = data["imu_params"]["Mg"][4].as<double>();
      calib.Mg(2,2) = data["imu_params"]["Mg"][5].as<double>();
      calib.Mg_inv = calib.Mg.inverse();

      calib.Ma(0,0) = data["imu_params"]["Ma"][0].as<double>();
      calib.Ma(1,0) = data["imu_params"]["Ma"][1].as<double>();
      calib.Ma(1,1) = data["imu_params"]["Ma"][2].as<double>();
      calib.Ma(2,0) = data["imu_params"]["Ma"][3].as<double>();
      calib.Ma(2,1) = data["imu_params"]["Ma"][4].as<double>();
      calib.Ma(2,2) = data["imu_params"]["Ma"][5].as<double>();
      calib.Ma_inv = calib.Ma.inverse();
    }
  }
  else
  {
                          ...
                          const double timeout_sec)
{
         ...
         img_timestamp_sec - this->imu_calib_.delay_imu_cam)
  {
    ...
    wait_time.resume();
    // wait
    VLOG(300) << "Waiting for imu measurements.";
  }

  return true;
}

void ImuHandler::interpolate_ang_vels(
  const ImuMeasurements& imu_measurements,
  const std::deque<double>& interpolation_times,
  std::deque<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& interpolated_vec)
{
  size_t n = interpolation_times.size();
  size_t it_i = 0;
  size_t t_i = imu_measurements.size() - 1;

  while (true)
  {
    double t1 = imu_measurements[t_i].timestamp_;
    double t2 = imu_measurements[t_i - 1].timestamp_;

    if( (interpolation_times[it_i] >= t1) && 
    (interpolation_times[it_i] < t2) )
    {
        Eigen::VectorXd v1 = imu_measurements[t_i].angular_velocity_;
        Eigen::VectorXd v2 = imu_measurements[t_i - 1].angular_velocity_;

        double dt = (interpolation_times[it_i] - t1) / (t2 - t1);

        Eigen::VectorXd v = v1 + dt * (v2 - v1);

        interpolated_vec.push_back(v);
...
void ImuHandler::interpolate_lin_accels(
  const ImuMeasurements& imu_measurements,
  const std::deque<double>& interpolation_times,
  std::deque<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>& interpolated_vec)
{
  ...
  while (true)
  {
    ...
    (interpolation_times[it_i] < t2) )
    {
      Eigen::VectorXd v1 = imu_measurements[t_i].linear_acceleration_;
      Eigen::VectorXd v2 = imu_measurements[t_i - 1].linear_acceleration_;
...
void ImuHandler::interpolate_measurements(
  const ImuMeasurements& imu_measurements,
  const std::deque<double>& interpolation_times,
  ImuMeasurements& interpolated_vec)
{
  size_t n = interpolation_times.size();
  size_t it_i = 0;
  size_t t_i = imu_measurements.size() - 1;
  ...
  while (true)
  {
    ...
    (interpolation_times[it_i] < t2) )
    {
      ...
      Eigen::VectorXd a1 = imu_measurements[t_i].linear_acceleration_;
      Eigen::VectorXd a2 = imu_measurements[t_i - 1].linear_acceleration_;

      double dt = (interpolation_times[it_i] - t1) / (t2 - t1);

      Eigen::VectorXd v = v1 + dt * (v2 - v1);
      Eigen::VectorXd a = a1 + dt * (a2 - a1);

      ImuMeasurement m;
      m.timestamp_ = interpolation_times[it_i];
      m.angular_velocity_= v;
      m.linear_acceleration_= a;

      interpolated_vec.push_back(m);
